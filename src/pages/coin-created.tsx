...
const CoinCreated: NextPage = () => {


  const [successMessage, setSuccessMessage] = useState("");
  const [errorMessage, setErrorMessage] = useState("");
  const [coin, setCoin] = useState<Currency | undefined>(undefined);
  //const router = useRouter();
  const { push,query } = useRouter();
  const [logged, setLogged] = useState<Boolean>(false);

  const loading = !!!coin;
  const coinId = Number(query.cupomId)


  console.log(query.cupomId);
  console.log(coinId);
  ...
  useEffect(() => {
    ...
    console.log(coinId);
    console.log(query.cupomId);
    console.log(Number(query.cupomId));
    if (!loading) {
      return undefined;
    }

    if(!!coinId){
      currencyService.getCurrencyById(coinId).then((res)=>{
        console.log(res);
        setCoin(res)
      }).catch((erro)=>{
        if(!!setErrorMessage){
  ...
  const onEditHandler = (currency: Currency, changes: any, alertMessage: string) => {
      ...
      currencyService.updateCurrency(currency, changes)
          .then((_res: Currency) => {
  ...
  return (
    <div>
      <Head>
        <title>{!!coin?`${coin?.acronym}: ${coin?.hid}`:""}</title>
      ...
      {isLoading || loading ? <CenteredComponent>
          ...
          <HomePageItem>
            ...
            <Column>
              <ItemProperty>
                <ShareButtons link={window.location.href} bannerLink={coin?.photo || coin?.logo}/>
              </ItemProperty>
            </Column>
          </HomePageItem>
          <HomePageItem>
            ...
            <Column>
              <ItemProperty>
                <PromoImage entity={coin} bookmaker={bookmaker || undefined} onEditHandler={onEditHandler}/>
              </ItemProperty>
            </Column>
          </HomePageItem>
          <HomePageItem>
            ...
            <Column>
              <ItemProperty>
                <AvatarImage entity={coin} bookmaker={bookmaker || undefined} onEditHandler={onEditHandler}/>
              </ItemProperty>
            </Column>
          </HomePageItem>
          <HomePageItem>
            <ItemProperty>
              <VoucherRules voucher={coin} title={"Leia as Regras (Importante!)"}/> {/* descricao (nao editavel) */}
            </ItemProperty>
          </HomePageItem>
          <HomePageItem>
            <ItemProperty>
              <CurrencyData currency={coin} bookmaker={bookmaker || undefined} title={"Dados do Vale Cupom"}/> {/* informacoes editaveis do voucer (talvez seja visivel apenas para o owner) */}
            </ItemProperty>
          </HomePageItem>
          {/* <HomePageItem>
